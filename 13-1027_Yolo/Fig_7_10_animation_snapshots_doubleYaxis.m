function Fig_7_10_animation_snapshots_doubleYaxis(cfg)
close all
tic
...
% font_name = 'Calibri';
font_name = 'Helvetica';
leg_ext = 1.0;   % this can be 1.2 for Calibri font; stretches the legned box a little
fidc = fopen(cfg);
...
%%   
    ...
    xlabel(sprintf('%s: %s',item_name,datestr(cur_date,'mmm dd, yyyy HH:MM')),'fontsize',font_size)
    ...
    title(cbar_axes,sprintf('Gate Close 4/30 %s (ft)',item_name),'fontsize',font_size,'fontname',font_name);
    set(cbar_axes,'fontsize',font_size,'fontname',font_name);

%% Time series: Figure 3

    set(gcf,'CurrentAxes',s3)
    [AX,H1,H2] = plotyy(TS_PO_time,Data_FreQ_alt,TS_PO_time(1:6:end),VON_Q_1D{1});
    
    axes(AX(2))
    hold on
    H3 = plot(AX(2),TS_PO_time(1:6:end),Gate_totalQ,'g');
    set(H2,'color','r')
    set(AX,{'ycolor'},{'k';'k'})

    for xn = 1:length(AX)
        set(AX(xn),'xlim',[datenum([model_year-1 10 1]) datenum([model_year 7 1])])
        set(AX(xn),'fontsize',font_size,'fontname',font_name)
        datetick(AX(xn),'x','mmm','keeplimits')
        set(AX(xn),'xticklabel',[]);
    end
    title(AX(1),sprintf('%s Modeled Flow Time Series',alt_type{kk}),'fontname',font_name,'fontsize',font_size)
    ...
    leg_s3 = legend([H1,H2,H3],'Fremont Weir','Sac R at Verona','Gate Close 4/30','location','northwest');
    leg3_pos = get(leg_s3,'position');
...
%         set(s3,'xticklabel',[]);
%         title(s3,sprintf('%s Modeled Flow Time Series',alt_type{kk}))
%         ylabel(s3,'Flow (cfs)')
...
%     y_lim = get(s3,'ylim');
    ...
    set(s3,'yticklabel',strtick,'fontname',font_name,'fontsize',font_size)
...
%         s4 = axes('position',[0.65 0.387 .3 .247]);
        ...
        leg4_pos = get(leg_s4,'position');
        set(leg_s4,'position',[leg4_pos(1:2) leg4_pos(3)*leg_ext leg4_pos(4)])

    
        title(s4,'Westside Tributary Inflows','fontname',font_name,'fontsize',font_size)
        datetick(s4,'x','mmm','keeplimits')
        set(s4,'xticklabel',[]);
        ylabel(s4,'Flow (cfs)','fontname',font_name,'fontsize',font_size)
    % end
    y_lim = get(s4,'ylim');
    ...
    set(s4,'yticklabel',strtick,'fontname',font_name)
...
%         set(s5,'xlim',[TS_trib_time(1) TS_trib_time(end)])
        ...
        leg5_pos = get(leg_s5,'position');
        set(leg_s5,'position',[leg5_pos(1:2) leg5_pos(3)*leg_ext leg5_pos(4)])
        
        
        title(s5,'Wetted Area Time Series','fontname',font_name,'fontsize',font_size)
        datetick(s5,'x','mmm','keeplimits')
        xlabel(s5,sprintf('WY %d',model_year),'fontname',font_name,'fontsize',font_size)
        ylabel(s5,'Wetted Area (acres)','fontname',font_name,'fontsize',font_size)
    % end
    y_lim = get(s5,'ylim');
...
fig_export_path = 'R:\Projects\13-1027_Yolo_Bypass_Fish_Passage\Reporting\YBM_Model_Report_Draft\Figures';

print(hf_1,'-djpeg','-r250',fullfile(fig_export_path,fig_export_name))
close(hf_1)
end
