...
%     TS_PO_time = addtodate(datenum([1996 10 2 0 0 0]),TS_PO(1),'hour') + dTS_PO./24;
    ...
    if ~isempty(intersect(modelYear(j),dry_yr))
        ...
        dry_mod{wy_ind} = VON_Data(ib);
        dry_obs{wy_ind} = vonQ_data(ia);
        
        clear wy_ind
    elseif ~isempty(intersect(modelYear(j),norm_yr))
        [~,wy_ind] = intersect(norm_yr,modelYear(j));
        nor_mod{wy_ind} = VON_Data(ib);
        nor_obs{wy_ind} = vonQ_data(ia);
        
        clear wy_ind
    else
        [~,wy_ind] = intersect(wet_yr,modelYear(j));
        wet_mod{wy_ind} = VON_Data(ib); 
        wet_obs{wy_ind} = vonQ_data(ia);
...
print(hf_1,'-dpng','Fig_6-3_Verona_Q')
