function Fig_7_9_Close_0301_Wetted_Area_withQ(WY)
close all
% if isempty(nargin)
%     WY = 2007;
% end
...
%% -------------------------------------------------- Gate close 4/1
file_name_mult = 1; % used in figure number sorting
...
file_Lg    = sprintf('WetAreaTabulation_yolo_200ft_Exg_TS6_____%d_{NearBypass}_FreLg01_Mar1.csv',WY);
file_Med   = sprintf('WetAreaTabulation_yolo_200ft_Exg_TS6_____%d_{NearBypass}_FreMed01_Mar1.csv',WY);
file_Sm    = sprintf('WetAreaTabulation_yolo_200ft_Exg_TS6_____%d_{NearBypass}_FreSm01_Mar1.csv',WY);
file_SacW  = sprintf('WetAreaTabulation_yolo_200ft_Exg_TS6_____%d_{NearBypass}_SacW01_Mar1.csv',WY);
...
print(h_fig,'-djpeg','-r300',sprintf('Fig_A7-%d_FreQ_WetA_Mar1',fig_num))
%close(h_fig)
...
% --------------------------------------------  Scenarios
% if WY == 1997
%     fpath_1D = sprintf('\\\\Cbecm11\\k\\Raw Results\\%da\\Mar1\\csv',WY);
% else
    fpath_1D = sprintf('\\\\Cbecm11\\k\\Raw Results\\%d\\Mar1\\csv',WY);
% end
% alt_type = {'FreSm';'FreMed';'FreLg';'SacW'}; % sample Alt type
...
if isempty(intersect(WY,group2))
    fname_1D_Gate = sprintf('yolo_200ft_%s01_TS6___Mar1_%d_1d_O.csv',alt_type,WY);
elseif isempty(intersect(WY,group1))
    fname_1D_Gate = sprintf('yolo_200ft_%s01_TS6_Mar1___%d_1d_O.csv',alt_type,WY);
end
% Gate Sim flows
...
fpath_1D = sprintf('\\\\Cbecm11\\k\\Raw Results\\%d\\Mar1\\csv',WY);
% alt_type = {'FreSm';'FreMed';'FreLg';'SacW'}; % sample Alt type
fname_1D_Gate = sprintf('yolo_200ft_%s01_TS6___Mar1%d_1d_O.csv',alt_type,WY);
