...
% 1999
cd('E:\Work\Projects\13-1027_Yolo_Bypass_Fish_Passage\LT Model\TUFLOW\results\1999\NearBypass\grids')
...
mov(1:nFrames) = struct('cdata', [],...
    'colormap', []);
vidObj = VideoWriter(sprintf('yolo_%s_%d.avi',item_name,model_year));
vidObj.Quality = 100;
vidObj.FrameRate = 4;
open(vidObj);
...
fname_PO = sprintf('yolo_200ft_FreSm01_TS6_____%d_PO.csv',model_year);

fidPO = fopen(fullfile(fpath_PO,fname_PO));
readfmt = ['%*f%f' repmat('%*f',1,23) '%f%*f%f%*f%*f%f'];
PO_Model_data = textscan(fidPO,readfmt,'delimiter',',','headerlines',3);
fclose(fidPO);

TS_PO = PO_Model_data{1};
TS_PO = TS_PO(TS_PO>=TS_start & TS_PO<=TS_end);
Data_FreWSE_w = PO_Model_data{2}(TS_PO>=TS_start & TS_PO<=TS_end);
Data_FreWSE_e = PO_Model_data{3}(TS_PO>=TS_start & TS_PO<=TS_end);
Data_FreQ = PO_Model_data{4}(TS_PO>=TS_start & TS_PO<=TS_end);
...
fclose(fidx);
    ...
    if i == 1
        ...
        % axis(s3,'tight')
        leg_s3 = legend(s3,'FRE-Sim','VON-Sim');
        datetick(s3,'x','mmm')
        set(s3,'xticklabel',[]);
