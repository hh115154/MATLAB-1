...
%% plot setup
% Figure template
fig_text.source = 'TUFLOW model output: LIY modeled and observed data';
...
fig_text.description = 'LIY Stage';
...
for j = 1:length(modelYear)
    ...
    fclose(fidQ);
    yby_Data = ybyH{1};  % 1: liy, 2: yby, 3: lis
...
%     TS_PO_time = addtodate(datenum([1996 10 2 0 0 0]),TS_PO(1),'hour') + dTS_PO./24;
    ...
    [C,ia,ib] = intersect(liyH_time,yby_time_model);
    scatter(curr_axis,liyH_data(ia),yby_Data(ib),5,line_color{j},'filled')
    hold(curr_axis,'on')
    grid(curr_axis,'on')
...
patch_xs = [t_x-.2 t_x-.2 end_point-.3 end_point-.3];
...
%         text(14.2,8,text_axes1,'VerticalAlignment','top','fontsize',8)
...
print(hf_1,'-dpng','Fig_6-6_LIY_WSE')
