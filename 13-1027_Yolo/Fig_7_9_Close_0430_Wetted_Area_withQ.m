function Fig_7_9_Close_0430_Wetted_Area_withQ(WY)
close all
% if isempty(nargin)
%     WY = 2007;
% end
...
%% -------------------------------------------------- Gate close 4/30
file_name_mult = 4; % used in figure number sorting
...
hold on
H3 = plot(AX(2),Gate_TS,GateSm,'r');
set(H1,'color','k')
set(H2,'color','k','linestyle',':')
set(AX,{'ycolor'},{'k';'k'})

H4 = plot(AX(2),Gate_TS,GateMed,'g');
H5 = plot(AX(2),Gate_TS,GateLg,'b');
H6 = plot(AX(2),Gate_TS,GateSacW,'m');
...
xlabel(h_axes1(2),sprintf('WY %d',WY),'fontsize',font_size)
...
xlabel_limit = [datenum([WY-1 10 1]) datenum([WY 7 1])];
...
set(h_axes1(1),'yticklabel',strtick,'fontname',font_name,'fontsize',font_size)
...
print(h_fig,'-djpeg','-r300',sprintf('Fig_A7-%d_FreQ_WetA_Apr30',fig_num))
close(h_fig)
...
fpath_1D = sprintf('\\\\Cbecm11\\k\\Raw Results\\%d\\csv',WY);
