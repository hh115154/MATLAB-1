...
for j = 1:length(modelYear)
    ...
    if ~isempty(intersect(modelYear(j),dry_yr))
        [~,wy_ind] = intersect(dry_yr,modelYear(j));
        dry_mod{wy_ind} = Data_FreQ(ib);
        dry_obs{wy_ind} = freQ_data(ia);
        
        clear wy_ind
    elseif ~isempty(intersect(modelYear(j),norm_yr))
        [~,wy_ind] = intersect(norm_yr,modelYear(j));
        nor_mod{wy_ind} = Data_FreQ(ib);
        nor_obs{wy_ind} = freQ_data(ia);
        
        clear wy_ind
    else
        [~,wy_ind] = intersect(wet_yr,modelYear(j));
        wet_mod{wy_ind} = Data_FreQ(ib); 
        wet_obs{wy_ind} = freQ_data(ia);
...
xlabel(h_axes1(3),'Observed Flow (cfs)','fontsize',8) % bottom figure
...
for k = 1:3  % number of subplots
    ...
    ylabel(curr_axis,'Modeled Flow (cfs)','fontsize',8)
    ...
    if k == 1
        text_axes1 = {sprintf('R2 = %.3f',R2_dry.rsquare); ...
            sprintf('RMSE = %s',commaint(RMSE_dry))};
    ...
    elseif k == 2
        ...
        text_axes1 = {sprintf('R2 = %.3f',R2_norm.rsquare); ...
            sprintf('RMSE = %s',commaint(RMSE_norm))};
        ...
        text_axes1 = {sprintf('R2 = %.3f',R2_wet.rsquare); ...
            sprintf('RMSE = %s',commaint(RMSE_wet))};
        % text(20.3,13,text_axes1,'VerticalAlignment','top','fontsize',8)
...
% title(h_axes1(1),sprintf('Dry years: R^2 = %.3f, RMSE = %s',R2_dry.rsquare,commaint(RMSE_dry)))
% title(h_axes1(2),sprintf('Normal years: R^2 = %.3f, RMSE = %s',R2_norm.rsquare,commaint(RMSE_norm)))
% title(h_axes1(3),sprintf('Wet years: R^2 = %.3f, RMSE = %s',R2_wet.rsquare,commaint(RMSE_wet)))
...
%% printing figure
print(hf_1,'-dpng','Fig_6-1_Fremont_Q')
