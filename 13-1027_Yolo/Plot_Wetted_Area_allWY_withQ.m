function Plot_Wetted_Area_allWY_withQ(WY)
close all
% if isempty(nargin)
%     WY = 2007;
% end
...
grid(s1,'on')
legend(s1,{'Fremont Weir','Westside Tribs','Gate FreSm', ...
...
% -----------------------------------------  lower panel
plot(s2,ts_exist,wetA_exist,'color','k')
hold(s2,'on')
plot(s2,ts_Sm,wetA_Sm,'color','r')
plot(s2,ts_Med,wetA_Med,'color','g') %previous 'y'
plot(s2,ts_Lg,wetA_Lg,'color','b') %previous 'g'
plot(s2,ts_SacW,wetA_SacW,'color','m')

grid(s2,'on')
legend(s2,{'Existing','FreSm','FreMed','FreLrg','SacW'},'location','northwest')
