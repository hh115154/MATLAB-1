...
%% read data
fid = fopen('Figure5_14_data.txt');
D = textscan(fid,'%f%f%f','headerlines',1);
fclose(fid);
...
label_y = interp1([6.651e3;D{1}],[20;maxd],label_st); 
%% plotting
...
fix = 0.03;
h2 = axes('position',[leg_pos(1)+fix leg_pos(2)-0.05 leg_pos(3)-fix leg_pos(4)-.02],'units','normalized','ActivePositionProperty','position');
...
print_name = sprintf('Fig_5-14_r300_test_%s_%s',figure_type,orientation);
print(hf_1,'-djpeg','-r250',print_name)
