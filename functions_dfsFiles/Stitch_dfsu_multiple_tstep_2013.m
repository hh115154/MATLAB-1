...
Loop_start_time = 1;
Loop_end_time = TOTAL_TIME;

% TOTAL_TIME = 10;  % hardwired
% Loop_start_time = Header_p.time_step_all - TOTAL_TIME +1;
% Loop_end_time = Header_p.time_step_all;
%% --------------------------------------------------------------
...
item_stitched = zeros(1,length(Header.mesh_info(6).data));

for iter = Loop_start_time:Loop_end_time            % for loop for time steps
    ...
    for nitem = 1:Header.Item_info.number         % for loop for items
        for i = 1:length(dir_info)  % for loop for partial files
            ...
            [Header_partial,~,~] = Locate_Data_Block_2012(dfsu_name);
            % cross-check: the function test
            item1_struc = fun_ReadItemTimeStep(fid_part,nitem,iter-1);
%             item1_fun{i} = item1_struc.data;
            
            % Put data back in order: v2012 changed how partial files are
            % written. Partial elements now has a serial number.
            item_stitched(Header_partial.mesh_info(6).data) = item1_struc.data;
