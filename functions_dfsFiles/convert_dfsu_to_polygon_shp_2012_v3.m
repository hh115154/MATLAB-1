function convert_dfsu_to_polygon_shp_2012_v3(items)

% 10/30/12 sjb, convert all time steps at once
...
% [filepath, filename, ~] = fileparts(infile);
...
fprintf('Took %.0f seconds to calculate node values\n',toc)


for j = 1:dfsu2.NumberOfTimeSteps
    tic
    % Reading DATA ---------------------------------
    h = double(dfsu2.ReadItemTimeStep(items,j-1).Data)'; %Item 1 = surface elevation
    ...
    shp_name = sprintf('%s\\%s_%s_t_%d.shp',shp_folder,filename(1:end-5),item_name,j);
    
    shapewrite(S,fullfile(filepath,shp_name))
    
    fprintf('Took %.0f seconds to convert t = %d\n',toc,j)
