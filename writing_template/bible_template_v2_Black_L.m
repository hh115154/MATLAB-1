...
% v2: 3/18/2015 - template for pages on LEFT side
clear all; close all
...
p_greyTOP = patch([.5 .5 PaperW-Rmargin-.5 PaperW-Rmargin-.5]./PaperW, ...
    [PaperH-Tmargin-dy Bmargin Bmargin PaperH-Tmargin-dy]./PaperH,[.5 .5 .5]);
...
%     [PaperH-Tmargin+dy/2 Pos_Y_line(end)+dy/2 Pos_Y_line(end)+dy/2 PaperH-Tmargin+dy/2]./PaperH,[.5 .5 .5]);
% set(p_greyTOP1,'edgecolor','none')
p_greyTOP1 = patch([1 1 1+4/3 1+4/3]./PaperW, ...
    [PaperH-Tmargin+dy/2 Pos_Y_line(end)+dy/2 Pos_Y_line(end)+dy/2 PaperH-Tmargin+dy/2]./PaperH,[.5 .5 .5]);
set(p_greyTOP1,'edgecolor','none')
...
p_greyTOP2 = patch([7.5-1/3 7.5-1/3 7.5 7.5]./PaperW, ...
    [PaperH-Tmargin+dy/2 Pos_Y_line(end)+dy/2 Pos_Y_line(end)+dy/2 PaperH-Tmargin+dy/2]./PaperH,[.5 .5 .5]);
set(p_greyTOP2,'edgecolor','none')
...
% horizontal guideline for Book names and page number (60% of the top box)
% line([.5 PaperW-.5]./PaperW,[PaperH-Tmargin+dy/2 PaperH-Tmargin+dy/2]./PaperH, ...
%     'Parent',axes1,'LineWidth',2,'Color','w')
% 
% line([.5 PaperW-.5]./PaperW,[PaperH-Tmargin-dy/2 PaperH-Tmargin-dy/2]./PaperH, ...
%     'Parent',axes1,'LineWidth',2,'Color','g')

line([.5 PaperW-.5]./PaperW,[PaperH-Tmargin-dy/2+dy*.6 PaperH-Tmargin-dy/2+dy*.6]./PaperH, ...
    'Parent',axes1,'LineWidth',2,'Color','w')

%% Bottom black (for dates)
p_greyBOT1 = patch([1 1 2 2]./PaperW, ...
    [Bmargin-1/10 Bmargin-1/10-1/6 Bmargin-1/10-1/6 Bmargin-1/10]./PaperH,[.5 .5 .5]);
...
p_greyLeft = patch([Lmargin+dx Lmargin+dx Lmargin+dx*2 Lmargin+dx*2]./PaperW, ...
    [PaperH-Tmargin-dy Pos_Y_line(1)-dy Pos_Y_line(1)-dy PaperH-Tmargin-dy]./PaperH,'w');
...
text(0.5,(PaperH - dy*2.5)/PaperH,'EVEN pages','horizontalalignment','center','fontsize',14)

%% print the page
print(hf_1,'-dpdf','-r600','test_2_L_grey')
