function varargout = Storm_ID_GUI_Main_B(varargin)
...
% Last Modified by GUIDE v2.5 10-Aug-2011 16:28:54
...
if strcmp(handles.InputSetup.BSF_period{1},'All')
    ...
    title(handles.axes_DWF,sprintf('%s (%s): %s~%s',handles.InputSetup.st_name,handles.InputSetup.st_num, ...
        datestr([2011 handles.InputSetup.BSF_period{1} 1 0 0 0],'mmm'), ...
        datestr([2011 handles.InputSetup.BSF_period{2} 1 0 0 0],'mmm')))
end
...
if ~StormSpecific
    ...
    if length(SMColon) == 1
        ...
        % remove last rows with 'Maximum','Minimum' etc
        for kk = length(C_stege{1})-4:length(C_stege{1})
            lastdatarow = kk-1;
            if isempty(str2num(C_stege{1}{kk}(1)))
               break 
            end
        end
        
        C_valid{1} = C_stege{1}(1:lastdatarow);
        C_valid{2} = C_stege{2}(1:lastdatarow);
        C_valid{3} = C_stege{3}(1:lastdatarow);
        
        ind_C_nan = isnan(C_valid{2});  
        
        C{1} = C_valid{1}(~ind_C_nan);
        C{2} = C_valid{2}(~ind_C_nan);
        C{3} = C_valid{3}(~ind_C_nan);
...
%         IN.FM.iInterfaceDataTimeInterval = dGap;
    ...
    if length(SMColon) == 1  % hardwired to Stege project
        ...
        tsEqu_vec = datevec(tsEqu);
        
        startYear = tsEqu_vec(1,1);
        endYear = tsEqu_vec(end,1);
        allYear = startYear:1:endYear;
        ...
        if strcmp(IN.BSF_period{1},'All')   % if BSF period is selected 'all'
            ...
            if IN.BSF_period{2} < IN.BSF_period{1}
                block_months = 12 + IN.BSF_period{2} - IN.BSF_period{1} + 1;
            else
                block_months = IN.BSF_period{2} - IN.BSF_period{1} + 1;
            end
            
            for yr = 1:length(allYear)
                %  block_start(yr) = datenum([allYear(yr) IN.BSF_period{1} 1 0 0 0]);
                %  block_end(yr) = addtodate(block_start(yr),block_months,'month');
                block_startend(yr,:) = [datenum([allYear(yr) IN.BSF_period{1} 1 0 0 0])  ...
                    addtodate(datenum([allYear(yr) IN.BSF_period{1} 1 0 0 0]),block_months,'month')];
                
            end
            % BSF_period now has only 'months' numbers for starting and ending
...
if all(ind_nan)  % incase the selected period contains nothing but NaN!
    FM = FMraw;
    RG = RGraw;
else
...
if SummerOnly
 ...
 %{
    ...
    % ====================================================================
 %}
%     FM = FM(indSelection);   % could be months-block through multiple years (non-continuous)
%     RG = RG(indSelection);
%     tsEqu = tsEqu(indSelection);
    
    if isnumeric(IN.BSF_period{1})
        DW_index = 0;
        for iter = 1:length(allYear)
            StartSelection = block_startend(iter,1);
            EndSelection =  block_startend(iter,2);
            Sel_t_int = find(tsEqu>=StartSelection & tsEqu<EndSelection);  % exclude the last data (at 00:00)
            Sel_t_raw = find(tsRaw>=StartSelection & tsRaw<EndSelection);
            
            if ~isempty(Sel_t_int)
                DW_index = DW_index+1;
                DW_block(DW_index) = calc_DW(IN,RG(Sel_t_int),FM(Sel_t_int),FMraw(Sel_t_raw),tsEqu(Sel_t_int),tsRaw(Sel_t_raw),dGap,StartSelection,EndSelection,SummerOnly);
                
            end
        end
        % AND get average DW from blocks
        
        
        if ~exist('DW_block')  % if data selected is NOT available from the raw data file (i.e. data too short, etc.)
            errordlg('DATA selection error: Select a valid data range')
        else
            DW_fieldnames = fieldnames(DW_block);
            for iterf = 1:length(DW_fieldnames)
                DW.(DW_fieldnames{iterf}) =  nanmean([DW_block.(DW_fieldnames{iterf})],2);
            end
            %
            %         mean([DW_block.AverageBaseDSF]);
            %         mean([DW_block.WeekendAvg],2)
            disp('debug')
        end
    else  % if All data is used    
        
        DW = calc_DW(IN,RG,FM,FMraw,tsEqu,tsRaw,dGap,StartSelection,EndSelection,SummerOnly);
    end
...
function DW = calc_DW(IN,RG,FM,FMraw,tsEqu,tsRaw,dGap,StartSelection,EndSelection,SummerOnly)
...
% ------------------------------ step 1 & 3
if length(indHrRG) ~= round(EndSelection-StartSelection)
...
% ------------------------------ step 2
if length(indHrFM) ~= round(EndSelection-StartSelection)
    fprintf(1,'Date missing from original data\n')
end
...
% identify Storm event during a specified DRY period

% Start time of selected storm
start_selected_storm = datevec(handles.table1_stats(StormRind,1));
% find storms during the specified DRY period
if isnumeric(handles.InputSetup.BSF_period{1})
    if handles.InputSetup.BSF_period{1}>handles.InputSetup.BSF_period{2}
        Storm_in_DRY = start_selected_storm(:,2)>=handles.InputSetup.BSF_period{1} | start_selected_storm(:,2)<=handles.InputSetup.BSF_period{2};
    else
        Storm_in_DRY = start_selected_storm(:,2)>=handles.InputSetup.BSF_period{1} & start_selected_storm(:,2)<=handles.InputSetup.BSF_period{2};
    end
else  % if {'All'  'All'}
    Storm_in_DRY = logical(zeros(size(start_selected_storm,1),1)+1);  % make all 'true' logical variable for the number of selected storm
end

%  -------- flip the axes
if get(handles.checkbox_data_months,'value') == 1
    
    s1 = scatter(handles.axes_Rfactor,cell2mat(RfactorTable(~Storm_in_DRY,4)),cell2mat(RfactorTable(~Storm_in_DRY,2)),50,'square','b','filled');
    hold(handles.axes_Rfactor,'on')
    
    s2 = scatter(handles.axes_Rfactor,cell2mat(RfactorTable(Storm_in_DRY,4)),cell2mat(RfactorTable(Storm_in_DRY,2)),50,[0 0.5 0],'filled');
else
    scatter(handles.axes_Rfactor,cell2mat(RfactorTable(:,4)),cell2mat(RfactorTable(:,2)),'square','b','filled');
end
xlabel(handles.axes_Rfactor,'Rainfall (MG)')
ylabel(handles.axes_Rfactor,'RDI/I Volume (MG)')
title(handles.axes_Rfactor,sprintf('RDI/I Separation: Avg R %.2f%%',mean(cell2mat(RfactorTable(:,6)))))
grid(handles.axes_Rfactor,'on')
xlim_Rfactor = get(handles.axes_Rfactor,'xlim');
set(handles.axes_Rfactor,'xlim',[0 xlim_Rfactor(2)])

hold(handles.axes_Rfactor,'off')

handles.Storm_in_DRY = Storm_in_DRY;
guidata(hObject, handles);
...
%     setup_GUI = guidata(Storm_ID_GUI_Setup_B);
%     disp(setup_GUI);
end
...
set(handles.uitable_FM_UpStrm, 'Data', handles.EmptyTable3);
set(handles.text_all_data_sign,'visible','on')
...
Storm_ID_GUI_Setup_B('Storm_ID_GUI_Main_B', handles.figure1);
...
if ~isempty(StormRind)
    ...
    % write dry weather period
    xlswrite(fullfile(PathExcel,filename_excel),{'Data';'Used for';'Dry Diurnal';'Pattern'},'Parameters','A13');
    if isnumeric(handles.InputSetup.BSF_period{1})
        xlswrite(fullfile(PathExcel,filename_excel),{'From','To'},'Parameters','B12');
        xlswrite(fullfile(PathExcel,filename_excel),handles.InputSetup.BSF_period,'Parameters','B13');
    else
        xlswrite(fullfile(PathExcel,filename_excel),{'All Available data'},'Parameters','B13');
    end
...
%     xlswrite(fullfile(PathExcel,filename_excel),{'DBSF'},'Diurnal','A1');
    ...
    xlswrite(fullfile(PathExcel,filename_excel),{'Storm occured in'},'R-factor','N1');
    
    
    % tag 'dry' storms
    for i=1:length(handles.Storm_in_DRY)
        Storm_time{i,1} = 'wet';
    end
    
    if isnumeric(handles.InputSetup.BSF_period{1})
        Storm_time(handles.Storm_in_DRY,1) = {'dry'};
    else
        Storm_time(handles.Storm_in_DRY,1) = {'All'};
    end
    
    
    xlswrite(fullfile(PathExcel,filename_excel),Storm_time,'R-factor','N2');
...
% --- Executes on button press in checkbox_data_months.
function checkbox_data_months_Callback(hObject, eventdata, handles)
% hObject    handle to checkbox_data_months (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of checkbox_data_months
% disp('check box clicked')

if isfield(handles,'Storm_in_DRY') % if 'Storm_in_DRY' field exists
