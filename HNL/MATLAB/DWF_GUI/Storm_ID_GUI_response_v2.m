function varargout = Storm_ID_GUI_response_v2(varargin)
...
% Last Modified by GUIDE v2.5 07-Jan-2011 16:02:42
...
%     handles = rmfield(handles,'EmptyTable');
% end
...
if round(mode(diff(FM{3}(1:2)))*60*24) ~= handles.Input.RG.dt
    dGap = round(mode(diff(FM{3}(1:2)))*60*24);  % 'mode' of dt in data
...
% Total Volume
stats{i,5} = sum(handles.FM{4}(StormIndices)) * ...
    handles.Input.FM.iInterfaceDataTimeInterval /24/60;
% Storm Volume
stats{i,6} = stats{i,5} - sum(BSFtseries(DiurnalInd).*BSF.AverageBaseDSF)/24;
