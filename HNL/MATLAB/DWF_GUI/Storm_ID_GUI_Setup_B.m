function varargout = Storm_ID_GUI_Setup_B(varargin)
% STORM_ID_GUI_SETUP_B MATLAB code for Storm_ID_GUI_Setup_B.fig
%      STORM_ID_GUI_SETUP_B, by itself, creates a new STORM_ID_GUI_SETUP_B or raises the existing
%      singleton*.
%
%      H = STORM_ID_GUI_SETUP_B returns the handle to a new STORM_ID_GUI_SETUP_B or the handle to
%      the existing singleton*.
%
%      STORM_ID_GUI_SETUP_B('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in STORM_ID_GUI_SETUP_B.M with the given input arguments.
%
%      STORM_ID_GUI_SETUP_B('Property','Value',...) creates a new STORM_ID_GUI_SETUP_B or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Storm_ID_GUI_Setup_B_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Storm_ID_GUI_Setup_B_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Storm_ID_GUI_Setup_B

% Last Modified by GUIDE v2.5 10-Aug-2011 16:32:25

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Storm_ID_GUI_Setup_B_OpeningFcn, ...
                   'gui_OutputFcn',  @Storm_ID_GUI_Setup_B_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Storm_ID_GUI_Setup_B is made visible.
function Storm_ID_GUI_Setup_B_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Storm_ID_GUI_Setup_B (see VARARGIN)

% Choose default command line output for Storm_ID_GUI_Setup_B
handles.output = hObject;

mainGuiInput = find(strcmp(varargin, 'Storm_ID_GUI_Main_B'));

% THIS IS WHERE IT OPNES THE MAIN GUI INDEPENDENTLY  :: FIX THIS!!!!
% ---------------------------------------------------------
% handles.MainGUI = Storm_ID_GUI_Main_B;  % stores handles of Main GUI
...
% UIWAIT makes Storm_ID_GUI_Setup_B wait for user response (see UIRESUME)
% uiwait(handles.figure_setup);


% --- Outputs from this function are returned to the command line.
function varargout = Storm_ID_GUI_Setup_B_OutputFcn(hObject, eventdata, handles) 
...
% MainGUIhandles = Storm_ID_GUI_Main_B;
...
period_ind = [get(handles.radiobutton_BSF1,'value') get(handles.radiobutton_BSF3,'value')];
...
%         
%         set(handles.popupmenu_start_month,'String','All')
%         set(handles.popupmenu_end_month,'String','All')
        
        handles.Input.BSF_period = {'All' 'All'};
    case 2  % user-selection
        
%{
         set(handles.edit_BSF_start,'String',datestr([2010 7 1 0 0 0]))
         set(handles.edit_BSF_end,'String',datestr([2010 10 1 0 0 0]))
         
         set(handles.popupmenu_start_month,'value',4)
         set(handles.popupmenu_end_month,'value',9)
     otherwise
...
%}
        ...
        str_start = get(handles.popupmenu_start_month,'String');
        str_end = get(handles.popupmenu_end_month,'String');
        
        val_start = get(handles.popupmenu_start_month,'value');
        val_end = get(handles.popupmenu_end_month,'value');
        
        % e.g. {'Jul' 'Sep'}
%         handles.Input.BSF_period = {str_start{val_start} str_end{val_end}};
        % e.g. {7 9}
        handles.Input.BSF_period = {val_start val_end};
        
        all_months = get(handles.popupmenu_start_month,'string');
        % change text on checkbox (Main window)
        set(mainHandles.checkbox_data_months,'String', ...
            sprintf('Data from %s~%s',all_months{val_start},all_months{val_end}))
...
guidata(Storm_ID_GUI_Main_B, mainHandles);
...
switch get(eventdata.NewValue,'Tag') % Get Tag of selected object.
    case 'radiobutton_BSF1'
        ...
        set(handles.popupmenu_start_month,'visible','off')
        set(handles.text_tilde,'visible','off')
        set(handles.popupmenu_end_month,'visible','off')
        
        set(mainHandles.checkbox_data_months,'visible','off')
        set(mainHandles.text_all_data_sign,'visible','on')
    ...
    case 'radiobutton_BSF3'
        ...
        set(handles.popupmenu_start_month,'visible','on')
        set(handles.text_tilde,'visible','on')
        set(handles.popupmenu_end_month,'visible','on')
        
%         set(mainHandles.uipanel_dry_pattern,'visible','on')
        set(mainHandles.checkbox_data_months,'visible','on')
        set(mainHandles.checkbox_data_months,'value',1)

        set(mainHandles.text_all_data_sign,'visible','off')
...
%         set(handles.edit_BSF_end,'String',datestr([2010 10 31 0 0 0],'mm-dd-yyyy'))
        default_start = 7;
        default_end = 9;
        set(handles.popupmenu_start_month,'value',default_start)
        set(handles.popupmenu_end_month,'value',default_end)
        
        
        all_months = get(handles.popupmenu_start_month,'string');
        % change text on checkbox in Main window
        % this is an initial default value of July~Sept
        set(mainHandles.checkbox_data_months,'String', ...
            sprintf('Data from %s~%s',all_months{default_start},all_months{default_end}))
...
% --- Executes on selection change in popupmenu_start_month.
function popupmenu_start_month_Callback(hObject, eventdata, handles)
% hObject    handle to popupmenu_start_month (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns popupmenu_start_month contents as cell array
%        contents{get(hObject,'Value')} returns selected item from popupmenu_start_month


% --- Executes during object creation, after setting all properties.
function popupmenu_start_month_CreateFcn(hObject, eventdata, handles)
% hObject    handle to popupmenu_start_month (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on selection change in popupmenu_end_month.
function popupmenu_end_month_Callback(hObject, eventdata, handles)
% hObject    handle to popupmenu_end_month (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns popupmenu_end_month contents as cell array
%        contents{get(hObject,'Value')} returns selected item from popupmenu_end_month


% --- Executes during object creation, after setting all properties.
function popupmenu_end_month_CreateFcn(hObject, eventdata, handles)
% hObject    handle to popupmenu_end_month (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
