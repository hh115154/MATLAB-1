function [RGcell,FMcell,DW] = fun_MDW_Calibrate_Telog(IN)

global st_name st_num
%% Read input file
...
set(Hf_2,'Units','pixel','Position',[120 100 800 600]);
...
FigName2 = sprintf('%s_%s_DWF',st_num,st_name);
print(Hf_2,'-dpng',fullfile(PathName,FigName2))
   
%% Plot Rain
Hf_3 = figure('NumberTitle','off','Name',sprintf('Rain %s: %s',st_name,st_num));
set(Hf_3,'Units','pixel','Position',[110 100 800 600]);
...
FigName3 = sprintf('%s_%s_Rain_FM',st_num,st_name);
print(Hf_3,'-dpng',fullfile(PathName,FigName3))
...
%% Write DWF to ASCII
DWF_ASCII_name = sprintf('%s_%s_DWF_table.txt',st_num,st_name);
fidw = fopen(fullfile(PathName,DWF_ASCII_name),'w');
fprintf(fidw,'TITLE\n');
fprintf(fidw,'%s at %s\n',st_name,st_num);
