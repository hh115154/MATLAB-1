function Jflow_figure_S5_200yr
...
tif_folder_S = 'J:\M2_EdriveCopy\Work\13-1009_SRV_Jflow_Ph2\UpperandFeather200yrPhase1\UpperFeather200yInterGrids.7z';
...
Figure_TITLE = 'Scenario 5 (Yolo Widen) - 200-year';
...
% ++++
flow_path_S = 'J:\M2_EdriveCopy\Work\13-1009_SRV_Jflow_Ph2\UpperandFeather200yrPhase1\UpperandFeather200yrMaxandMonitor';
...
% ++++
flow_path_SY = 'J:\M2_EdriveCopy\Work\13-1009_SRV_Jflow_Ph2\Scenario5\Yolo Only Simulations\Yolo200yrScenario5MaxMonitor';
...
flow_file_Yolo11_12_S3 = 'YoloQ_Scenario5_200YR.csv';
...
feat_loc = 'R:\Projects\13-1009_JFLOW_Phase2\GIS\DM_GIS\Scenario5_Features_SB';
line_shp_1 = 'ID34_NL.shp';
line_shp_2 = 'ID35_NL_a.shp';
line_shp_3 = 'ID35_NL_b.shp';
line_shp_4 = 'ID32_NL_line.shp';
line_shp_5 = 'ID32a_NL.shp';



pgon_shp_1 = 'ID33_OL.shp';
pgon_shp_2 = 'ID34_OL.shp'; 
pgon_shp_3 = 'ID35_breach.shp';
pgon_shp_4 = 'ID36_weir.shp';
pgon_shp_5 = 'ID37_OL.shp'; 
pgon_shp_6 = 'ID37a_gate.shp';     % black color
pgon_shp_7 = 'ID37a_weir.shp';
pgon_shp_8 = 'Fweir.shp'; 
pgon_shp_9 = 'ID32_OL.shp';

pgon_shp_10 = 'ID35_OL_a.shp';
pgon_shp_11 = 'ID35_OL_b.shp';
...
% vidObj = VideoWriter(fullfile(ani_folder_loc,sprintf('Jflow_%s_200yr.avi',sce_name)));
% vidObj.Quality = 100;
% vidObj.FrameRate = 4;
% open(vidObj);
...
fig_width = 1500;
fig_height = 870;
hf_1 = figure('Color','w','Position',[50 100 fig_width fig_height]);
PaperW = 15.5;
PaperH = 9;

% margins - pixel

L_marg = 8;  % left margin
ani_gap = 7;
ani_W = 290;
ani_H = 740;
R_marg = 35;  % right margin
...
subplots(:,[1 3]) = subplots(:,[1 3])./fig_width;
subplots(:,[2 4]) = subplots(:,[2 4])./fig_height;
...
for i = 1:6
    eval(sprintf('s%d=axes(''units'',''normalized'',''position'',subplots(%d,:));',i,i))
end
...
pn_y = 'J:\M2_EdriveCopy\Work\13-1009_SRV_Jflow_Ph2\Scenario5\Yolo Only Simulations\Yolo200yrScenario5InterDepthGrids';
...
leg_s6 = legend(s6,[H1,H2,H3,L3(1),L3(2)],{'11','12','12 Stage','13','14'},'location','northwest');
leg6_pos = get(leg_s6,'position');
set(leg_s6,'position',[leg6_pos(1) leg6_pos(2)*.97 leg6_pos(3)*leg_ext leg6_pos(4)])
...
% for k = 31:33    
    ...
    % 3rd panel - Yolo
    plot_tiff_2(fullfile(pn_y,fnY),s3,s2,xlimN3,ylimN3)
    ...
    set(cbar_axes,'units','normalized');
    cbar_axes_size = [L_marg bot_5+ani_H+Gap_colorbar L_marg+ani_W1+ani_W2+ani_W3+ani_gap*0.6 5];
    cbar_axes_size([1 3]) = cbar_axes_size([1 3])./fig_width;
    cbar_axes_size([2 4]) = cbar_axes_size([2 4])./fig_height;
    
    set(cbar_axes,'position',cbar_axes_size);
    ...
    %set(cbar_axes,'fontsize',font_size,'fontname',font_name);
    set(cbar_axes,'fontsize',font_size)
    set(cbar_axes,'fontname',font_name)
...
% right panel
draw_scenario_features(s3,feat_loc,line_shp_1);
draw_scenario_features(s3,feat_loc,line_shp_2,'meter');
draw_scenario_features(s3,feat_loc,line_shp_3,'meter');
draw_scenario_features(s3,feat_loc,line_shp_4);
draw_scenario_features(s3,feat_loc,line_shp_5,'meter');

draw_scenario_features(s3,feat_loc,pgon_shp_1);
draw_scenario_features(s3,feat_loc,pgon_shp_2);
draw_scenario_features(s3,feat_loc,pgon_shp_3);
draw_scenario_features(s3,feat_loc,pgon_shp_4);
draw_scenario_features(s3,feat_loc,pgon_shp_5);
draw_scenario_features(s3,feat_loc,pgon_shp_7);
draw_scenario_features(s3,feat_loc,pgon_shp_8);
draw_scenario_features(s3,feat_loc,pgon_shp_9);

draw_scenario_features(s3,feat_loc,pgon_shp_10);
draw_scenario_features(s3,feat_loc,pgon_shp_11);

% gate
draw_scenario_features(s3,feat_loc,pgon_shp_6);   % black


% middle pane
draw_scenario_features(s2,feat_loc,line_shp_1);
draw_scenario_features(s2,feat_loc,line_shp_2,'meter');
draw_scenario_features(s2,feat_loc,line_shp_3,'meter');
draw_scenario_features(s2,feat_loc,line_shp_4);
draw_scenario_features(s2,feat_loc,line_shp_5,'meter');

draw_scenario_features(s2,feat_loc,pgon_shp_1);
draw_scenario_features(s2,feat_loc,pgon_shp_2);
draw_scenario_features(s2,feat_loc,pgon_shp_3);
draw_scenario_features(s2,feat_loc,pgon_shp_4);
draw_scenario_features(s2,feat_loc,pgon_shp_5);
draw_scenario_features(s2,feat_loc,pgon_shp_7);
draw_scenario_features(s2,feat_loc,pgon_shp_8);
draw_scenario_features(s2,feat_loc,pgon_shp_9);

draw_scenario_features(s2,feat_loc,pgon_shp_10);
draw_scenario_features(s2,feat_loc,pgon_shp_11);

% gate
draw_scenario_features(s2,feat_loc,pgon_shp_6);   % black
...
%                                  -------------------------------- whiskers    
    ...
    text(80,325000,{'Solid = Existing';'Dashed = Scenario'}, ...
    ...
    x_legend = [586500 608300];
    y_legend = [4290270  4297770];
...
%     text(2.0158376e6, 1.39995768e7,'Existing')
    ...
    x_text = repmat(x_legend(1)+4000,4,1);
    y_text = y_legend(1) + 1500.*[1:4];
    y_text = y_text';
    y_text = flipud(y_text);
    
    text(x_text,y_text,{'Monitoring Line';'New Levee';'Levee Degrade';'Existing Max Inundation'}, ...
        'fontsize',font_size,'fontname',font_name)
...
%     x_lim = [XLogoLeft(1)+1000 XLogoRight(1)-1000];
%     dx=(x_lim(2)-x_lim(1))/(size(logo,2)-1);
%     x_data=x_lim(1):dx:x_lim(2);
%     
%     y_lim = [YLogo(1)+1000 YLogo(2)-1000];
%     dy = (y_lim(2)-y_lim(1))/(size(logo,1)-1);
%     y_data=y_lim(1):dy:y_lim(2);
%     
%     image('Parent',s1,'CData',flipdim(logo,1),'XData',x_data,'YData',y_data);
% image(x,y,flipdim(logo,1));
%     
    
    
    %% Snapshot at t = 300
%     if k == 100
%        grab_fn = fullfile(ani_folder_loc,sprintf('%s_200yr_%dhr',sce_name,round(t_hour(k))));
%        % export_fig(grab_fn,'-png','-nocrop','-r600','-transparent','-append',hf_1)
%        export_fig(grab_fn,'-png','-nocrop','-r300','-transparent','-append',hf_1)
%     end
...
% set(hf_1,'PaperPosition',[0.3 0.45 16 6.25],'PaperSize',[16.3 7],'PaperOrientation','portrait')
% set(hf_1,'PaperPosition',[0 0 15.5 9],'PaperSize',[15.5 9],'PaperOrientation','portrait')  % fit for 11x17 template
set(hf_1,'PaperPosition',[0 0 fig_W fig_H],'PaperSize',[fig_W fig_H],'PaperOrientation','portrait')

print(hf_1,'-dpng','-r300',fullfile(ani_folder_loc,sprintf('Jflow_%s_200yr_normalized',sce_name)))
print(hf_1,'-dpdf','-r300',fullfile(ani_folder_loc,sprintf('Jflow_%s_200yr_normalized',sce_name)))
...
if yLim1(1) < 4.23e6
    fn = 'J:\M2_EdriveCopy\Work\13-1009_SRV_Jflow_Ph2\Yolo Only Phase 1 Tests\Yolo200yrPhase1MaxMonitor\D25200101.tif';
else
    fn = 'J:\M2_EdriveCopy\Work\13-1009_SRV_Jflow_Ph2\UpperandFeather200yrPhase1\UpperandFeather200yrMaxandMonitor\D2720002.tif';
end
GRID_ex = imread(fn);
Iex = geotiffinfo(fn);
...
function plot_tiff_2(fn,ax_h1,ax_h0,xLim1,yLim1,ax_h2,xLim2,yLim2)
global color_Max image_name
%% Color definition
load('YoloColormapsBlue','mycmap')
...
%% --------------------------------------- max inundation extent (exiting)
if yLim1(1) < 4.23e6
    fn = 'J:\M2_EdriveCopy\Work\13-1009_SRV_Jflow_Ph2\Yolo Only Phase 1 Tests\Yolo10yrPhase1MaxMonitors\D25010101.tif';
else
    fn = 'J:\M2_EdriveCopy\Work\13-1009_SRV_Jflow_Ph2\UpperandFeather10yrPhase1\UpperandFeather10yrMaxandMonitor\D2701001.tif';
end
GRID_ex = imread(fn);
Iex = geotiffinfo(fn);
...
for ii=1:cdivs
    ...
    plot(ax_h1,xcoord(idx),ycoord(idx),'.','MarkerSize',dot_size,'Color',cmap(ii,:));
    hold(ax_h1,'on')
    plot(ax_h0,xcoord(idx),ycoord(idx),'.','MarkerSize',dot_size,'Color',cmap(ii,:));
    hold(ax_h0,'on')
end
axis(ax_h1,'equal')
...
if nargin > 5
    ...
    % ====================================== Yolo portion (tiny northern tip)
    ...
    switch S(1).Geometry
        ...
        case 'Polygon'
            ...
            for q = 1:length(S)
                ...
                % set(p1,'edgecolor','none')
                if ~isempty(strfind(fn,'gate'))
                    shp_color = 'k';
                else
                    shp_color = [255 165 0]./255;
                end
                p1 = plot(ax_h,S(q).X(1:end-1).*factor,S(q).Y(1:end-1).*factor);
                set(p1,'linewidth',2,'color',shp_color)
                hold(ax_h,'on')
            end
