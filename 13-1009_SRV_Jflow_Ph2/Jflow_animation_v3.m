function Jflow_animation_v3
global color_Max image_name
close all

%% variables

tif_folder_S = 'C:\Work\13-1009_SRV_Jflow_Ph2\Scenario 3';
...
flow_path_S = 'C:\Work\13-1009_SRV_Jflow_Ph2\Scenario 3\Flow_data';
flow_file_S = 'cross_section_flow.csv';

flow_path_E = 'C:\Work\13-1009_SRV_Jflow_Ph2\Existing\Flow_data';
...
for p = 1:length(p_ind3)
    L3(p) = plot(s6,tsE,QEcfs(:,p_ind3(p)),[color_lines{p} type_lines{1}]);
    hold(s6,'on')
    plot(s6,tsS,QScfs(:,p_ind3(p)),[color_lines{p} type_lines{2}])
        
    leg_item3{2*p-1} = sprintf('%s %s',num2str(p_ind3(p)),'Ex');
    leg_item3{2*p} = sprintf('%s %s',num2str(p_ind3(p)),sce_name2);
end
set(s6,'xlim',[tsE(1) tsE(end)])
y_lim3 = get(s6,'ylim');
...
for k = 1:length(D_sce)
    ...
    xlabel(s3,sprintf('%d hour',tStamp(k)),'fontsize',font_size,'fontname',font_name)
    ...
    for kk = 1:length(Sn)
        ...
        l4 = line([tStamp(k) tStamp(k)],[0 y_lim1(2)],'color','k');
        
        set(gcf,'CurrentAxes',s5)
        % y_lim_max = max(y_lim(2),y_lim2(2));
        l5 = line([tStamp(k) tStamp(k)],[0 y_lim2(2)],'color','k');
        
        
        set(gcf,'CurrentAxes',s6)
        % y_lim_max = max(y_lim(2),y_lim2(2));
        l6 = line([tStamp(k) tStamp(k)],[0 y_lim3(2)],'color','k');
