function fun_ReplaceMeshValueInPolygon(cfg)
...
% [Header, loc_data, loc_time, loc_meshinfo] = fun_ReadDfsuHeader_v2012(fido);

% nitem = 1; % WSE 1st item usually
% 
% dfsu_WSE = fun_ReadItemTimeStep(fido,nitem,str2double(NA_t_IC));
% data_WSE = dfsu_WSE.data;

% delete_value = data_WSE(1);  % define delete value - not ideal

% ---------------- All non-delete values (wet cells) => ZERO
% data_WSE(data_WSE ~= data_WSE(1)) = 0; 
...
%% write a NEW mesh
new_mesh_name = sprintf('%s_shp%d.mesh',fn_mesh(1:end-5),item_shp);
...
% 
% 
% fidt = fopen(fullfile(NA_path_IC_dfsu,NA_fn_IC_dfsu));  % open template file
% [Headert, loc_datat, loc_timet, loc_meshinfot] = fun_ReadDfsuHeader_v2012(fidt);
% header_before_data = fread(fidt,loc_datat+4,'*int8');
% 
% 
% out_IC = sprintf('Tracer_IC_%s_area%d.dfsu',fn_shp(1:end-4),item_shp);
% 
% if exist(fullfile(NA_path_IC_dfsu,out_IC),'file') ~= 0
%     delete(fullfile(NA_path_IC_dfsu,out_IC));
% end
% 
% fidw = fopen(fullfile(NA_path_IC_dfsu,out_IC),'a+');
% fwrite(fidw,header_before_data,'int8');
% 
% fwrite(fidw,1,'int8');
% fwrite(fidw,Headert.Item_info.datasize,'int32');  % define data length
% fwrite(fidw,data_WSE,'float32');  % raw data

fclose('all');
% copyfile(fn_IC_dfsu,out_IC);
toc
